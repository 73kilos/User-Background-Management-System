<template>
  <div class="page-edituser">
    <el-form label-width="350px" @submit="updateDetail">
      ...
      <el-form-item>
        <el-button type="success" @click="updateDetail">提交</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
...
<script>
  export default {
    ...
    methods: {
      ...
      updateDetail(e) {
        e.preventDefault()
        if (!this.users.username || !this.users.password || !this.users.tel || !this.users.age || !this.users.description || !this.users.sex) {
       ...
       // 提交数据
        this.$http
          .put('http://localhost:3000/users/' + this.$route.params.id, newUser)
          // 提示用户提交成功
          .then(this.$message({
            message: '信息修改成功',
            type: 'success'
